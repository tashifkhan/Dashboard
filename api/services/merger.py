...
def merge_stat_lists(list_a: list[StatEntry], list_b: list[StatEntry]) -> list[StatEntry]:
    """
    ...
    for entry in list_a + list_b:
        if entry.key in merged:
            ...
            merged[entry.key].pageviews += entry.pageviews
            merged[entry.key].visitors += entry.visitors
        else:
            # Create a new copy to avoid modifying the original
            merged[entry.key] = StatEntry(**entry.model_dump())
...
def merge_timeseries(
    list_a: list[TimeseriesEntry], 
    list_b: list[TimeseriesEntry]
...
def merge_stats(vercel_stats: Stats, posthog_stats: dict[str, list[StatEntry]]) -> Stats:
    """
    ...
    return Stats(
        ...
        device_type=merge_stat_lists(vercel_stats.device_type, posthog_stats.get("device_type", [])),
        referrer=merge_stat_lists(vercel_stats.referrer, posthog_stats.get("referrer", [])),
        os_name=merge_stat_lists(vercel_stats.os_name, posthog_stats.get("os_name", [])),
        country=merge_stat_lists(vercel_stats.country, posthog_stats.get("country", [])),
    )
