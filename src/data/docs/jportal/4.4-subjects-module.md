...
> Source: https://deepwiki.com/codeblech/jportal/4.4-subjects-module
...
For attendance tracking per subject, see [Attendance Module](/codeblech/jportal/4.1-attendance-module). For exam schedules by subject, see [Exams Module](/codeblech/jportal/4.3-exams-module).
...
The `w` prop contains either a `WebPortal` or `MockWebPortal` instance, determined by authentication mode. This abstraction is set up in the parent `AuthenticatedApp` component (see [Application Structure & Authentication](/codeblech/jportal/3.1-application-structure-and-authentication)).

### Shared State

The Subjects Module shares `semestersData` state with other modules:

* **Attendance Module** ([#4.1](/codeblech/jportal/4.1-attendance-module)) may use similar semester selection
* **Grades Module** ([#4.2](/codeblech/jportal/4.2-grades-module)) uses separate semester lists but similar patterns
