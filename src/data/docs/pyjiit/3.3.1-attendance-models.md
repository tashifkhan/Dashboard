...
> Source: https://deepwiki.com/codelif/pyjiit/3.3.1-attendance-models
...
For information about how these models are used in API calls, see [Webportal Class](/codelif/pyjiit/3.1-webportal-class). For other data models in the library, see [Exam Models](/codelif/pyjiit/3.3.2-exam-models), [Registration Models](/codelif/pyjiit/3.3.3-registration-models), and [Tokens and Captcha](/codelif/pyjiit/3.3.4-tokens-and-captcha).
...
* As input to `Webportal.get_registered_subjects_and_faculties()` (see [Registration Models](/codelif/pyjiit/3.3.3-registration-models))
...
The `header` and `sem` objects are passed to `Webportal.get_attendance()`, which uses their fields to construct the API request. The method returns a dictionary with attendance records (see [Webportal Class](/codelif/pyjiit/3.1-webportal-class) for details).
...
* `NotLoggedIn` exception if `get_attendance_meta()` is called without authentication (see [Exception Handling](/codelif/pyjiit/3.4-exception-handling))
