...
> Source: https://deepwiki.com/codelif/pyjiit/1.1-system-architecture
...
For detailed information about the encryption implementation, see [Security and Encryption](/codelif/pyjiit/4-security-and-encryption). For installation and basic usage, see [Getting Started](/codelif/pyjiit/2-getting-started). For complete API method documentation, see [Core API Reference](/codelif/pyjiit/3-core-api-reference).
...
def get_attendance_meta(self):
    # Method body only executes if self.session is not None
```
```
...
```
```
resp = self.__hit("POST", API+endpoint, data=payload, exception=LoginError)
```
