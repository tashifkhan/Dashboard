...
For general web content extraction and processing, see [Web Search and Content Processing](3.3.2-web-search-and-content-processing). For AI agent tool usage patterns, see [Agent Tool System](4.3-agent-tool-system).
...
A LangChain prompt template is constructed using the prompt injection detection template and invoked with the markdown content. The chain uses the global `llm` instance from the LLM abstraction layer (see [LLM Integration Layer](4.5-llm-integration-layer)).
