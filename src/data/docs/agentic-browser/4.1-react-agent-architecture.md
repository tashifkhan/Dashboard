...
For information about the individual tools available to the agent, see [Agent Tool System](4.3-agent-tool-system). For the Browser Use Agent that generates automation scripts, see [Browser Use Agent and Script Generation](4.2-browser-use-agent-and-script-generation). For LLM provider configuration, see [LLM Integration Layer](4.5-llm-integration-layer).
