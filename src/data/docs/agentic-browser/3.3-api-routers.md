...
| `github_router` | `/api/genai/github` | `POST /` | GitHub repository Q&A and analysis | [3.3.4](3.3.4-github-integration) |
| `website_router` | `/api/genai/website` | `POST /` | Web page content analysis and Q&A | [3.3.2](3.3.2-web-search-and-content-processing) |
| `youtube_router` | `/api/genai/youtube` | `POST /` | YouTube video transcript Q&A | [3.3.3](3.3.3-youtube-integration) |
| `google_search_router` | `/api/google-search` | `GET /` | Web search via Tavily API | [3.3.2](3.3.2-web-search-and-content-processing) |
| `gmail_router` | `/api/gmail` | `POST /unread` `POST /latest` `POST /mark_read` `POST /send` | Gmail operations with OAuth | [3.3.1](3.3.1-gmail-and-calendar-integration) |
| `calendar_router` | `/api/calendar` | `POST /events` `POST /create` | Google Calendar operations with OAuth | [3.3.1](3.3.1-gmail-and-calendar-integration) |
| `pyjiit_router` | `/api/pyjiit` | Multiple endpoints | JIIT Webportal integration | [3.3.5](3.3.5-pyjiit-webportal-integration) |
| `react_agent_router` | `/api/genai/react` | `POST /` | Conversational AI with tool use | [4.1](4.1-react-agent-architecture) |
| `website_validator_router` | `/api/validator` | `POST /` | Prompt injection detection | [3.3.6](3.3.6-website-validator) |
| `browser_use_router` | `/api/agent` | `POST /` | Browser automation script generation | [4.2](4.2-browser-use-agent-and-script-generation) |
...
The PyJIIT router uses an encrypted session token obtained through a separate authentication flow. Details in [3.3.5](3.3.5-pyjiit-webportal-integration).
...
Two routers provide AI agent capabilities and are documented in detail in [Chapter 4](4-agent-intelligence-system):

* **React Agent Router** (`/api/genai/react`): Implements conversational AI with dynamic tool selection using LangGraph orchestration. See [4.1](4.1-react-agent-architecture).
* **Browser Use Router** (`/api/agent`): Generates structured JSON action plans for browser automation. See [4.2](4.2-browser-use-agent-and-script-generation).

These routers represent the intelligence layer of the system, leveraging the LLM abstraction layer documented in [4.5](4.5-llm-integration-layer).
