...
This page documents the Python backend project structure, dependency management system, and the core `LargeLanguageModel` abstraction layer that enables multi-provider LLM support. This covers the foundational configuration and infrastructure for the FastAPI backend. For details on the FastAPI application itself and its routers, see [FastAPI Application](3.2-fastapi-application-and-router-architecture). For information about individual service integrations, see [Service Integration Routers](3.3-api-routers).
