...
| --- | --- | --- | --- |
| Frontend | `oven/bun:1` → `oven/bun:1-slim` | Multi-stage build | Slim runner image, production deps only |
...
2. **nginxproxyman\_network**: External network for Nginx Proxy Manager integration (see [Docker Compose Setup](6.1-docker-compose-setup))
